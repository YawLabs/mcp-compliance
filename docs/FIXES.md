...
In both eras, a `transport-batch-reject` or `transport-content-type-reject` failure that says `not evaluable` is not about your batch or media-type handling: something in front of the server answered the probe in its place -- a 401 or an auth-gate 403 (any Bearer challenge without `--auth`, one carrying an `error` parameter with it: pass or fix `--auth`; a gateway's `-32001` body does not make it yours), a 429 that survived one retry, or any other 403 next to a conformant twin that could not get past the gate either (a gateway refusing every request; when the details quote `Invalid Host` / `Invalid Origin`, allow the hostname you tested through). The twin is the same request sent without the defect -- the same `ping` sent on its own as `application/json` on 2025-11-25, a conformant `server/discover` on 2026-07-28 -- resent once when it draws a 429. A 403 counts as your rejection, whatever its message says, only when the twin was served or drew a status your server chose (a 2xx, or a 4xx other than 401, 403 or 429); a twin that drew the same 403, a 401, a second 429, a 5xx or no answer credits nothing. On 2026-07-28 any rejection is also not evaluable when the setup `server/discover` was itself rejected: a server that rejects everything proves nothing by rejecting the probe too, so fix `lifecycle-discover` first. Reject a batch or a wrong Content-Type with a 4xx, never a 5xx: in both eras a 5xx fails as the server failing on the request (a broken server, or a gateway with no backend). `transport-batch-reject` still credits a 5xx that carries your own `-32600`, with a warning about the status; a 5xx with `-32603`, a server-defined code or no JSON-RPC error fails. On 2026-07-28 a probe that gets no answer at all, the resend after a 429 included, fails as `server unreachable`.
...
On 2025-11-25 a failure that says `not evaluable` is not about your envelope: the `initialize` was answered without a result by something in front of your server, so the envelope need not be yours -- a 401 or an auth-gate 403 (pass or fix `--auth`), a 429 from a rate limiter, a 5xx without your own `-32600`, `-32601` or `-32602` (a broken server, or a gateway with no backend), or a 403 next to the same `ping` sent on its own that could not get past the gate either -- it drew the same 403, a 401, a second 429, a 5xx or no answer (when the details quote `Invalid Host` / `Invalid Origin`, allow the hostname you tested through). Fix `lifecycle-init` first.

On 2026-07-28 the same holds for the setup `server/discover` answered without a result (it is resent once after a 429, and every lifecycle rule judges the answer it settles on): a 401 or an auth-gate 403, a 429 that survived that resend, a 5xx without your own `-32600`, `-32601`, `-32602`, `-32020`, `-32021` or `-32022`, and any 403 without a Bearer challenge (it refused the conformant request itself, so there is nothing left to compare it with) fail as not evaluable. Fix `lifecycle-discover` first.
...
A server that does not report progress should ignore the token, not fail the request because of it. Both eras blame the token only when the failure reproduces: the call without the token was served, and the call carrying it failed again when resent. On 2026-07-28 a connection your server closes, or a stdio exit, on the call counts as such a failure too -- including a stdio child that answers the call and exits right after (a plain `server/discover` follows every answered call) -- and the child is restarted, with a warning naming the call. A call without the token that fails differently does not clear the token: when your tool needs arguments the empty call lacks, it answers `-32602` without the token, and a server that exits or answers `-32603` on every call carrying the token still fails once the resent call fails the same way again. Valid progress notifications do not excuse a call you then fail. A check that could not tell -- a gate's 401, auth-gate 403 or 429 answered one of the calls, or a call got no answer at all (a timeout, a connection never established) -- is a skip, not a pass.
...
On 2025-11-25 a failure that says `not evaluable` means the error did not come from your dispatcher: either `initialize` was rejected the same way (fix `lifecycle-init` first), or a gate answered in the server's place -- a 401 or an auth-gate 403 (any Bearer challenge without `--auth`, one carrying an `error` parameter with it), a 429 that survived one retry, or any other 403 next to a `ping` with the same headers that could not get past the gate either: it drew the same 403, a 401, a second 429, a 5xx or no answer (a gateway that lets `initialize` through and refuses every other method; when the details quote `Invalid Host` / `Invalid Origin`, allow the hostname you tested through). A 5xx fails as the server failing on the request unless it carries your own `-32601`, which passes with a warning: answer an unknown method with a 4xx or a JSON-RPC error on a 2xx, never a 5xx. The 2026-07-28 check reads its answer the same way, with a conformant `server/discover` as the twin (a gateway's `-32001` on a 401 no longer passes it): a 5xx without `-32601` fails there too as the server failing on the request, and a `-32601` on a 5xx passes with a warning that the spec requires 404. `lifecycle-version-negotiate` reads its answer the same way, with the `initialize` handshake as its only comparison (a 403 next to a served handshake is yours) and your own `-32600` or `-32602` credited on a 5xx.
...
In both eras, `error-invalid-jsonrpc`, `error-invalid-json`, `error-missing-params` and `error-capability-gated` read their answer the way `error-unknown-method` does (on 2026-07-28 `error-invalid-cursor` too, and the bodiless 4xx that `error-parse-code` and `error-invalid-request-code` accept), so a failure that says `not evaluable` is not about your validation: the rejection did not come from your server. Either the conformant request was rejected the same way (fix `lifecycle-init`, or `lifecycle-discover` on 2026-07-28, first; `error-capability-gated` then also never saw which capabilities you declare), or a gate answered in the server's place -- a 401 or an auth-gate 403 (any Bearer challenge without `--auth`, one carrying an `error` parameter with it: pass or fix `--auth`; a gateway's `-32001` body does not make it yours), a 429 that survived one retry, or any other 403 next to a twin with the same headers (a `ping` on 2025-11-25, a conformant `server/discover` on 2026-07-28; for `error-invalid-cursor`, its list method without the cursor, so a gateway that refuses the list method itself is caught) that could not get past the gate either: it drew the same 403, a 401, a second 429, a 5xx or no answer (when the details quote `Invalid Host` / `Invalid Origin`, allow the hostname you tested through). A 5xx that does not carry your own code (`-32600` for the malformed message, `-32700` for invalid JSON, `-32602` for the missing tool name or the invalid cursor, `-32601` for an undeclared capability's method) fails as the server failing on the request, in both eras, and one that carries it passes with a warning: reject bad input with a 4xx or a JSON-RPC error on a 2xx, never a 5xx.
...
A payload that crashes the server fails the test in both suites, naming the payload: a stdio server that exits on it (`server died on payload "$(whoami)" sent to echo.data: ...`), or an HTTP server that drops the connection and then answers no follow-up request (`server may have crashed: connection dropped on payload ...`). That is the bug to fix -- an unhandled throw in a tool handler takes a Node stdio server down with it. Both suites restart a stdio child that died this way (a warning names the check), every time a check's own request kills it, `--retries` included, so the checks after it measure your server rather than the crash. A drop the server outlives (a WAF or IPS dropping the request) counts as a payload that got no answer, with a warning, never as a rejection.
...
**Fix:** don't override Node's default stdout encoding (UTF-8). On Windows specifically, check `chcp` isn't set to a non-UTF-8 code page if you're spawning child processes. In both eras the failure names the evidence: U+FFFD replacement characters, a Latin-1 mis-decode, the non-ASCII characters replaced by `?` (a legacy code page's encoder), or the CJK/emoji characters dropped. A tool that splits its input into tokens passes as long as every non-ASCII piece comes back somewhere in the reply. When the tool does not echo its input, or you have no tool, the verdict rests on a request whose envelope carries the same characters -- a `ping` whose `_meta` carries them on 2025-11-25, a `server/discover` whose `clientInfo` name carries them on 2026-07-28 -- so a server that serves it passes. In both eras a server that exits on the probe, or right after answering it, fails and is restarted for the checks after it (a warning names the check): a plain request (`ping` on 2025-11-25, `server/discover` on 2026-07-28) is sent between the probes, and after the last answered probe the check waits up to 250 ms for an exit and then sends one, so a crash a moment after the answer is charged here and not to the next check. Each such request gets the per-request timeout; a server that answers and then stays silent for the whole `--timeout` fails as a hang (`... stopped answering right after`) and is named in a warning, but it is still running, so it is not restarted -- look for a reader thread or a logger that dies on the non-ASCII line. A server an earlier check had already killed fails as `server unreachable` instead, and the check to fix is the one that killed it.
...
If you advertise nothing, `-32601` for `subscriptions/listen` is fine. A failure that says `not evaluable` means the rejection was not yours: `server/discover` was rejected too, or a gate answered the listen in the server's place -- a 401 or an auth-gate 403 (a gateway's `-32001` included), a 429 that survived one retry, or any other 403 that the conformant `server/discover` could not get past either. A 5xx without `-32601` fails as the server failing on the listen, and when you advertise something a `-32601` on a 5xx fails as you refusing a method you advertise: answer a listen you do not support with a 4xx. The standalone HTTP GET stream is gone; answer GET with 405 (`transport-get-removed`).
