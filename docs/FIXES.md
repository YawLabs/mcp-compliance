...
On 2025-11-25, a `transport-batch-reject` or `transport-content-type-reject` failure that says `not evaluable` is not about your batch or media-type handling: something in front of the server answered the probe in its place -- a 401 or an auth-gate 403 (pass or fix `--auth`), a 429 that survived one retry, or a 403 that the same ping sent on its own as `application/json` drew too (a gateway refusing every request; when the details quote `Invalid Host` / `Invalid Origin`, allow the hostname you tested through). A 403 that ping did not draw counts as your rejection, whatever its message says. A 5xx fails as the server failing on the request: reject a batch or a wrong Content-Type with a 4xx. `transport-batch-reject` still credits a 5xx that carries your own `-32600`, with a warning about the status; a 5xx with `-32603`, a server-defined code or no JSON-RPC error fails.
...
On 2025-11-25 a failure that says `not evaluable` means the error did not come from your dispatcher: either `initialize` was rejected the same way (fix `lifecycle-init` first), or a gate answered in the server's place -- a 401 or an auth-gate 403, a 429 that survived one retry, or a 403 that a `ping` with the same headers drew too (a gateway that lets `initialize` through and refuses every other method; when the details quote `Invalid Host` / `Invalid Origin`, allow the hostname you tested through). A 5xx fails as the server failing on the request unless it carries your own `-32601`, which passes with a warning: answer an unknown method with a 4xx or a JSON-RPC error on a 2xx, never a 5xx. `lifecycle-version-negotiate` reads its answer the same way, with the `initialize` handshake as its only comparison (a 403 next to a served handshake is yours) and your own `-32600` or `-32602` credited on a 5xx.
