...
In both eras, a `transport-batch-reject` or `transport-content-type-reject` failure that says `not evaluable` is not about your batch or media-type handling: something in front of the server answered the probe in its place -- a 401 or an auth-gate 403 (pass or fix `--auth`; a gateway's `-32001` body does not make it yours), a 429 that survived one retry, or a 403 without a Bearer challenge next to a conformant twin that could not get past the gate either (a gateway refusing every request; when the details quote `Invalid Host` / `Invalid Origin`, allow the hostname you tested through). The twin is the same request sent without the defect -- the same `ping` sent on its own as `application/json` on 2025-11-25, a conformant `server/discover` on 2026-07-28 -- resent once when it draws a 429. A 403 counts as your rejection, whatever its message says, only when the twin was served or drew a status your server chose (a 2xx, or a 4xx other than 401, 403 or 429); a twin that drew the same 403, a 401, a second 429, a 5xx or no answer credits nothing. Reject a batch or a wrong Content-Type with a 4xx, never a 5xx: on 2025-11-25 a 5xx fails as the server failing on the request, and on 2026-07-28 as not evaluable (a server failure or a gateway with no backend). `transport-batch-reject` still credits a 5xx that carries your own `-32600`, with a warning about the status; a 5xx with `-32603`, a server-defined code or no JSON-RPC error fails.
...
In both eras, `error-invalid-jsonrpc`, `error-invalid-json`, `error-missing-params` and `error-capability-gated` read their answer the way `error-unknown-method` does (on 2026-07-28 `error-invalid-cursor` too), so a failure that says `not evaluable` is not about your validation: the rejection did not come from your server. Either the conformant request was rejected the same way (fix `lifecycle-init`, or `lifecycle-discover` on 2026-07-28, first; `error-capability-gated` then also never saw which capabilities you declare), or a gate answered in the server's place -- a 401 or an auth-gate 403 (pass or fix `--auth`; a gateway's `-32001` body does not make it yours), a 429 that survived one retry, or a 403 next to a twin with the same headers (a `ping` on 2025-11-25, a conformant `server/discover` on 2026-07-28) that could not get past the gate either: it drew the same 403, a 401, a second 429, a 5xx or no answer (when the details quote `Invalid Host` / `Invalid Origin`, allow the hostname you tested through). A 5xx that does not carry your own code (`-32600` for the malformed message, `-32700` for invalid JSON, `-32602` for the missing tool name or the invalid cursor, `-32601` for an undeclared capability's method) fails -- as the server failing on the request on 2025-11-25, as not evaluable on 2026-07-28 -- and one that carries it passes with a warning: reject bad input with a 4xx or a JSON-RPC error on a 2xx, never a 5xx.
...
**Fix:** don't override Node's default stdout encoding (UTF-8). On Windows specifically, check `chcp` isn't set to a non-UTF-8 code page if you're spawning child processes. In both eras the failure names the evidence: U+FFFD replacement characters, a Latin-1 mis-decode, the non-ASCII characters replaced by `?` (a legacy code page's encoder), or the CJK/emoji characters dropped. A tool that splits its input into tokens passes as long as every non-ASCII piece comes back somewhere in the reply. When the tool does not echo its input, or you have no tool, the verdict rests on a request whose envelope carries the same characters -- a `ping` whose `_meta` carries them on 2025-11-25, a `server/discover` whose `clientInfo` name carries them on 2026-07-28 -- so a server that serves it passes. In both eras a server that exits on the probe, or right after answering it (a plain request sent after each answered probe finds it gone), fails and is restarted for the checks after it (a warning names the check); a server an earlier check had already killed fails as `server unreachable` instead, and the check to fix is the one that killed it.
...
If you advertise nothing, `-32601` for `subscriptions/listen` is fine. A failure that says `not evaluable` means the rejection was not yours: `server/discover` was rejected too, or a gate answered the listen in the server's place -- a 401 or a Bearer 403 (a gateway's `-32001` included), a 429 that survived one retry, a 5xx without `-32601` (any 5xx when you advertise something), or a 403 without a Bearer challenge that the conformant `server/discover` could not get past either. The standalone HTTP GET stream is gone; answer GET with 405 (`transport-get-removed`).
