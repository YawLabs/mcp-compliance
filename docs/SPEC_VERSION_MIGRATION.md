...
2. Classify the reply:
   ...
   - an HTTP 401/403 without a modern error body -> **2025-11-25**, but the note says the era could not be determined and to pass `--auth`; when no `Authorization` header was configured the report's first warning also says the server requires authentication and the grade is not meaningful
