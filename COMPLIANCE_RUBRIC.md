...
- On stdio, a check whose own request kills the server (an injection payload, the 1 MB argument of `security-oversized-input`, the unknown arguments of `security-extra-params`, the unicode probe of `stdio-unicode` in both eras, the calls of `lifecycle-progress-token` on 2026-07-28, and on 2025-11-25 the second `initialize` of `lifecycle-version-negotiate`, which the details blame rather than its version) fails as died and restarts the server before it returns, with a warning naming the check. `stdio-unicode` and `lifecycle-progress-token` also charge a child that answers and exits right after: a plain request sent after the answered one (a `server/discover`, or a `ping` on 2025-11-25), with the per-request timeout, finds it gone, and `stdio-unicode` after its last probe, and `lifecycle-progress-token` after a call the server failed, first waits up to 250 ms for the exit. A child that answers and then stays silent for the whole `--timeout` is named in a warning and, still running, is not restarted (`stdio-unicode` fails as that hang). Every attempt that kills it restarts it again, so a retry never leaves the tests after it running against a dead process. `security-tool-rug-pull` restarts it the same way when its own `tools/call` between the two lists of an already restarted process kills it, and then skips.
...
- **Pass criteria:** Server returns HTTP 4xx status **or** a JSON-RPC error response (non-array body with `error` field) as its own rejection. A 429 is resent once after `Retry-After` (capped at 2 s) and the second answer decides. Any other 403 (one without a Bearer challenge) counts only when the same ping sent on its own (resent once after `Retry-After` when it draws a 429) was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429), whatever its message says (that ping is sent only in this case). A 5xx carrying the server's own `-32600` passes, with a warning that a rejected request should get a 4xx.
- **Fail criteria:** Server processes the batch and returns an array response, returns a 2xx status without an error, or returns a 5xx without the server's own `-32600` (a `-32603`, a server-defined code, or no JSON-RPC error). Not evaluable when a 401 or an auth-gate 403, a 429 on the retry as well, or a 403 its single-ping twin could not get past either answered in the server's place, JSON-RPC error body or not: the twin drew the same 403 (quoted when its message names Host/Origin validation), a 401, a 429 again, a 5xx, a redirect or no answer.
...
- **Pass criteria:** Server returns HTTP 4xx (ideally 415 Unsupported Media Type or 400 Bad Request) as its own rejection. A 429 is resent once after `Retry-After` (capped at 2 s) and the second answer decides. Any other 403 (one without a Bearer challenge) counts only when the same ping sent on its own as `application/json` (resent once after `Retry-After` when it draws a 429) was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429), whatever its message says (that ping is sent only in this case).
- **Fail criteria:** Server returns 2xx, attempts to parse the body, or returns 5xx. Not evaluable when something answered in the server's place: a 401 or an auth-gate 403, a 429 on the retry as well, or a 403 its `application/json` twin could not get past either: the twin drew the same 403 (quoted when its message names Host/Origin validation), a 401, a 429 again, a 5xx, a redirect or no answer.
...
- **Fail criteria:** The tool reply or the `ping` reply shows mangling: U+FFFD replacement characters, a Latin-1 mis-decode, the non-ASCII characters replaced by `?`, or the non-ASCII characters dropped (the probe's first word, or its ASCII skeleton, with neither the CJK word nor the emoji anywhere in the reply). Also fails when the tool call draws a `-32700` parse error, when the `ping` carrying the probe is answered with an error or a non-JSON-RPC reply, or when a probe that is sent gets no reply (a one-line reason) or is answered by a child that then exits or stops answering: a plain `ping` between the tool call and the envelope `ping` finds a child the tool call killed, and after the last answered probe the check waits up to 250 ms for the child to exit and, when it has not, sends a plain `ping`, which also finds an exit reported late. Each such `ping` gets the per-request timeout, so a server that answers within `--timeout` is never read as hung and a child that exits before `--timeout` runs out is charged; a child still running but silent for the whole `--timeout` fails as a hang (`... stopped answering right after (no reply to ping within Nms)`), with a warning naming the probe, and is not restarted. A child that exits on the probe, or right after answering it, is restarted with a fresh `initialize` handshake, with a warning naming the check, so the checks after it measure the server; a child already gone before it fails as server unreachable. Details keep to 220 characters: the note on the tool call gives way first, then the request, never the conclusion.
...
- **Pass criteria:** Response has `jsonrpc` equal to `"2.0"`, an `id` field, and either a `result` or `error` field. An `error` envelope counts only when it is the server's own; a 403 is, next to the same ping sent on its own as `application/json` (resent once after `Retry-After` when it draws a 429) that was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429). A server's own `-32600`, `-32601` or `-32602` refusal of the `initialize` on a 5xx is read as its envelope too, with a warning that a rejected request should get a 4xx.
- **Fail criteria:** Any of the three required JSON-RPC 2.0 fields is missing. An `initialize` that something in front of the server answered without a result fails as not evaluable, however valid the envelope (a gateway's `-32001` "Unauthorized" on its 401): a 401 or an auth-gate 403 (pass or fix `--auth`); a 429 (a rate limiter); a 5xx without the server's own refusal (a broken server, or a gateway with no backend); or any other 403 when the same ping sent on its own as `application/json` drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer (a guard refusing every request; a message naming Host/Origin validation is quoted when the ping drew the same 403). That ping is sent only for such a 403, at most once per run.
...
- **Pass criteria:** Server returns a JSON-RPC error response **or** HTTP 4xx status that is its own rejection of the body. Any other 403 (one without a Bearer challenge) counts only when a well-formed `ping` sent with the same headers (resent once on a 429) was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429). A 5xx carrying the server's own `-32700` passes, with a warning that a rejected request should get a 4xx.
- **Fail criteria:** Server returns neither a JSON-RPC error nor an HTTP 4xx status, or a 5xx without its own `-32700`. Not evaluable when the `initialize` handshake was not served and drew the same status (or no answer), when a 401 or an auth-gate 403, or a 429 on the retry as well, answered in the server's place, or when the well-formed `ping` drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer.
...
- **Transports:** http
- **Spec reference:** [basic/transports/streamable-http#sending-messages](https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/streamable-http#sending-messages)
- **Description:** Sends an otherwise valid server/discover (correct headers and _meta) with Content-Type: text/plain and expects a 4xx status. The POST body MUST be a single JSON-RPC message; a server that parses text/plain as JSON is trusting a content type it never checked. A rejection is credited only when it is the server's own: a 429 is resent once after Retry-After (capped at 2 s), and a 401, a 403 that reads as an auth refusal (an auth gate: without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after the resend, or any other 403 that a conformant server/discover sent next to it could not get past either fails as not evaluable. That twin is sent through the suite's client (the same credential and protocol headers) only for such a 403, is resent once on a 429, and credits the 403 only when it was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429); when it drew the same 403, a message naming Host/Origin validation is quoted. Any rejection -- a status >= 400, or a JSON-RPC error on a 2xx -- also fails as not evaluable when the conformant setup server/discover was itself rejected or got no response: a server that rejects everything proves nothing by rejecting text/plain too. When that setup server/discover already drew the probe's 403, its answer is the twin and no further server/discover is sent. Otherwise any 5xx fails as the server failing on the request rather than rejecting it (a broken server, or a gateway with no backend). A probe that gets no answer, or whose resend after a 429 gets none, fails as server unreachable.
- **Pass criteria:** HTTP 4xx (415 or 400 expected) for an otherwise valid server/discover sent with Content-Type: text/plain, as the server's own rejection, with the conformant setup server/discover served. A 429 is resent once after Retry-After (capped at 2 s), and any 403 that is not an auth refusal (one without a Bearer challenge, or with --auth one whose challenge carries no error parameter) is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** The text/plain body is parsed and answered with a 2xx status, or a redirect; or the POST gets no answer, the resend after a 429 included ("server unreachable"). A rejecting status fails as not evaluable when it is a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after its one resend, or any other 403 the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer), and any rejection (a status >= 400, or a JSON-RPC error on a 2xx) does when the conformant setup server/discover was itself rejected or got no response (when that discover drew the probe's 403, its answer is the twin and no further server/discover is sent). Otherwise any 5xx fails as the server failing on the POST rather than rejecting it (a broken server, or a gateway with no backend).
...
- **Transports:** http
- **Spec reference:** [basic/transports/streamable-http#sending-messages](https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/streamable-http#sending-messages)
- **Description:** POSTs a JSON array containing two valid server/discover requests and expects a 4xx status or a JSON-RPC error. The POST body MUST be a single JSON-RPC request or notification; batches were dropped in 2025-06-18 and remain unsupported. On a text/event-stream answer only JSON-RPC responses count: notifications before the error are ignored, and a stream that carries no response fails. An SSE data frame holding an array is the processed-batch shape, as the same array over application/json is, whatever its elements say. A rejection is credited only when it is the server's own: a 429 is resent once after Retry-After (capped at 2 s), and a 401, a 403 that reads as an auth refusal (an auth gate: without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after the resend, or any other 403 that a conformant server/discover sent next to it could not get past either fails as not evaluable. That twin is sent through the suite's client (the same credential and protocol headers) only for such a 403, is resent once on a 429, and credits the 403 only when it was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429); when it drew the same 403, a message naming Host/Origin validation is quoted. Any rejection -- a status >= 400, or a JSON-RPC error on any status (a gateway's -32001 on HTTP 200 included) -- also fails as not evaluable when the conformant setup server/discover was itself rejected or got no response, as the standard-header rules do: a legacy-only server pinned to this catalog, or a proxy answering every POST 404, proves nothing by rejecting the batch too, and a -32600 on a 5xx is then neither credited nor warned about. When that setup server/discover already drew the probe's 403, its answer is the twin and no further server/discover is sent. Otherwise a 5xx without -32600 fails as the server failing on the batch rather than rejecting it (a broken server, or a gateway with no backend), and a -32600 on a 5xx is credited, with a warning that a 4xx is expected. A probe that gets no answer, or whose resend after a 429 gets none, fails as server unreachable.
- **Pass criteria:** A JSON array of two valid server/discover requests draws HTTP 4xx or a JSON-RPC error body, as the server's own rejection, with the conformant setup server/discover served. A 429 is resent once after Retry-After (capped at 2 s), a -32600 on a 5xx passes with a warning that a 4xx is expected, and any 403 that is not an auth refusal (one without a Bearer challenge, or with --auth one whose challenge carries no error parameter) is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** A 2xx status without a JSON-RPC error, an array response (a JSON array body, or an SSE data frame holding an array), or a text/event-stream carrying no JSON-RPC response (notifications before an error frame are ignored); or the batch gets no answer, the resend after a 429 included ("server unreachable"). A rejecting status fails as not evaluable when it is a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after its one resend, or any other 403 the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer), and any rejection (a status >= 400, or a JSON-RPC error on any status) does when the conformant setup server/discover was itself rejected or got no response (a -32600 on a 5xx is then neither credited nor warned about; when that discover drew the probe's 403, its answer is the twin and no further server/discover is sent). Otherwise a 5xx without -32600 fails as the server failing on the batch rather than rejecting it (a broken server, or a gateway with no backend).
...
- **Description:** Sends server/discover with a conformant _meta and expects a result carrying a supportedVersions array and a capabilities object. Servers MUST implement server/discover; it replaces the initialize handshake and is the first response the suite trusts. Over HTTP a 429 on it is resent once after Retry-After (capped at 2 s), and the answer that exchange settles on is the one this rule, its lifecycle-discover-* siblings, lifecycle-jsonrpc, the capability gates and the negative probes' not-evaluable readings all judge; a resend that gets no response is server unreachable.
...
- **Description:** Validates the discover response envelope: jsonrpc is exactly '2.0', the id is echoed, and exactly one of result (an object) or error is present. All MCP messages MUST follow JSON-RPC 2.0 and a result response MUST include a result field. Over HTTP the setup server/discover it reads is resent once after Retry-After (capped at 2 s) when it draws a 429, and the answer that exchange settles on is judged (a pass then adds '; resent once after HTTP 429'); a resend that gets no response is server unreachable. The envelope of a discover answered without a result is judged only when the server wrote it: a 401, a 403 that reads as an auth refusal, any other 403 (it refused the conformant request itself, so nothing is left to compare it with; the message is quoted when it names Host/Origin validation), a 429 still a 429 after the resend, or a 5xx carrying none of -32600, -32601, -32602, -32020, -32021 or -32022 fails as not evaluable, because a gateway's envelope (its -32001 on a 401, echoing the id) proves nothing about the server's JSON-RPC. A 5xx carrying one of those codes is the server's own refusal and is credited, with a warning that a 4xx is expected. Over stdio every answer is the server's.
- **Pass criteria:** The server/discover response has jsonrpc exactly '2.0', the request id echoed, and exactly one of result (an object) or error. Over HTTP the setup server/discover that every lifecycle rule reads is resent once after Retry-After (capped at 2 s) when it draws a 429, and the answer it settles on is judged (the details add '; resent once after HTTP 429'); a server's own refusal code (-32600, -32601, -32602, -32020, -32021 or -32022) on a 5xx passes with a warning that a 4xx is expected.
- **Fail criteria:** A missing or wrong jsonrpc field, a missing id, both or neither of result/error, or a non-object result. Over HTTP a discover answered without a result fails as not evaluable when something in front of the server wrote the envelope: a 401, any 403 (an auth gate's, or one that refused the conformant request itself, so no twin is sent; a message naming Host/Origin validation is quoted), a 429 still a 429 after its one resend, or a 5xx without one of those codes (this rule's probe is the conformant request, so a 5xx on it is not read as the server failing on a defect). A resend that gets no response is server unreachable.
...
- **Description:** Calls the first tool without required arguments (preferring one whose name or description mentions progress, else the first listed tool) with _meta.progressToken set and reads the whole response; tools/list is fetched on demand when the tools tests did not run, and the test is skipped when the server declares no tools; when tools/list failed it skips pointing at tools-list if that test is in the run, and fails with the recorded reason when it is not. Progress is optional, so a call with no notifications/progress passes; but any progress notification that does arrive MUST carry the same token and its progress value MUST increase with each notification. A foreign token or a non-increasing value fails. A server may ignore the token but not fail the request because of it. A 429 on any of the check's calls is resent once after Retry-After (capped at 2 s). The call failing on the server's side -- a JSON-RPC error, an HTTP status >= 400 other than a 429 or an auth gate's 401 or 403 (without --auth any Bearer challenge, with --auth one carrying an error parameter), a connection the server closed, or a stdio child that exits on it -- is blamed on the token only once it is reproduced: the same call without the token, sent right after, is served, and the call carrying the token, resent after that, fails again (its progress notifications are judged like the first's). A tool whose first call fails whatever it carries (a cold backend) passes when the resent call is served. A failure that the server's own answer to the call without the token shares, or a resent call answered without failing again, stays a passing observation. Shared means the same JSON-RPC error code whatever the HTTP status (a server that streams a call carrying a token over SSE reports the error on a 200), or, when there is no JSON-RPC error, the same HTTP status. A call without the token that fails differently clears nothing (for example, a tool whose required arguments the empty call lacks answers -32602 without the token, and exits, closes the connection or answers -32603 with it). The call carrying the token is then resent. If it fails the way it first did (the same code, or an exit or closed connection again), the check fails. If it is served, or answered the way the call without the token was, it passes. Anything else (a gate's answer, no answer, a third kind of failure) is a skip. Progress notifications settle the check only on a call the server did not fail: a server that sends a valid notifications/progress and then fails every call carrying the token is read like one that sent none, and its notifications are named in the details. Only the server's own answers attribute anything: a gate's answer (a 429 still a 429 after its resend, a 401, an auth gate's 403) on any of the three calls, a call without the token that gets no response, or a first or resent call that gets none (a timeout, a connection never established, a stdio child already gone) measured nothing about the token, so the check is skipped, never a scored pass. On stdio a child that exits on one of the calls is restarted before the next, with a warning naming the call, so the calls and checks after it reach a live process. That includes a child that answers a call and exits right after: every answered call is followed by one plain server/discover (with the per-request timeout), and a call the server failed (a JSON-RPC error) is first given up to 250 ms for the child to exit. The exit is charged to that call, never to the call without the token or the resent call. A served call gets no such wait, so a child that serves the call and exits a moment after its server/discover is answered is not seen by this check. A child that stops answering after a call is named in a warning and left running. Details keep to 220 characters: the tool's name and the answers quoted give way, never the conclusion.
...
- **Default required:** Yes
- **Spec reference:** [basic/transports/streamable-http#protocol-version-header](https://modelcontextprotocol.io/specification/2026-07-28/basic/transports/streamable-http#protocol-version-header)
- **Description:** Sends a method name that does not exist (compliance/nonexistent) with a full modern _meta and expects a JSON-RPC error response echoing the request id. On HTTP the server MUST respond 404 Not Found together with the JSON-RPC error body, so a 404 + error passes, an error carried on another status (a 200, a 400) passes with a warning on the status, and a result fails. A rejection is credited only when the conformant server/discover was served; a server that rejects everything fails this test as not evaluable (its probe is not resent after a 429 then). It is also not evaluable when something in front of the server answered in its place: a 401, a 403 that reads as an auth refusal (an auth gate: without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after one resend after Retry-After (capped at 2 s), or any other 403 that a conformant server/discover sent next to the probe could not get past either. That twin is sent through the suite's client (the same credential and protocol headers) only for such a 403, is resent once on a 429, and credits the 403 only when it was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429); when it drew the same 403, a message naming Host/Origin validation is quoted. A 5xx without -32601 fails as the server failing on the probe rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable. A -32601 on a 5xx passes, with a warning that the spec requires 404.
- **Pass criteria:** A JSON-RPC error echoing the request id; on HTTP a 404 status passes cleanly and an error carried on another status (a 200, a 400) passes with a warning. A 429 is resent once after Retry-After (capped at 2 s), a -32601 on a 5xx passes with a warning that the spec requires 404, and any 403 that is not an auth refusal (one without a Bearer challenge, or with --auth one whose challenge carries no error parameter) is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** A result, no JSON-RPC error, no response, or an error that does not echo the request id. A server that rejects server/discover too fails as not evaluable, and so does an answer something in front of the server gave: a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after its one resend, or any other 403 the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer). A 429 is not resent while server/discover was rejected. A 5xx without -32601 fails as the server failing on the probe rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable.
...
- **Transports:** http
- **Spec reference:** [basic/index#requests](https://modelcontextprotocol.io/specification/2026-07-28/basic/index#requests)
- **Description:** POSTs a JSON object that is not a valid JSON-RPC request (no method, no id) with otherwise correct headers and expects a JSON-RPC error or a 4xx status. Requests MUST carry jsonrpc, method and a string or integer id; a result fails. A rejection is credited only when the conformant server/discover was served; a server that rejects everything fails this test as not evaluable (its probe is not resent after a 429 then). It is also not evaluable when something in front of the server answered in its place: a 401, a 403 that reads as an auth refusal (an auth gate: without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after one resend after Retry-After (capped at 2 s), or any other 403 that a conformant server/discover sent next to the probe could not get past either. That twin is sent through the suite's client (the same credential and protocol headers) only for such a 403, is resent once on a 429, and credits the 403 only when it was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429); when it drew the same 403, a message naming Host/Origin validation is quoted. A 5xx without -32600 fails as the server failing on the probe rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable. A -32600 on a 5xx passes, with a warning that a 4xx is expected.
- **Pass criteria:** A JSON object with no method and no id draws a JSON-RPC error or an HTTP 4xx status. A 429 is resent once after Retry-After (capped at 2 s), a -32600 on a 5xx passes with a warning that a 4xx is expected, and any 403 that is not an auth refusal (one without a Bearer challenge, or with --auth one whose challenge carries no error parameter) is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** A result, a status below 400 without a JSON-RPC error, or no response. A server that rejects server/discover too fails as not evaluable, and so does an answer something in front of the server gave: a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after its one resend, or any other 403 the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer). A 429 is not resent while server/discover was rejected. A 5xx without -32600 fails as the server failing on the probe rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable.
...
- **Transports:** http
- **Spec reference:** [basic/index#error-codes](https://modelcontextprotocol.io/specification/2026-07-28/basic/index#error-codes)
- **Description:** POSTs a body that is not JSON ('{not json') with the standard headers and expects a parse error (-32700) or a 4xx status. A hang, a result, or a status below 400 carrying no JSON-RPC error (an HTML page on a 200) fails. A rejection is credited only when the conformant server/discover was served; a server that rejects everything fails this test as not evaluable (its probe is not resent after a 429 then). It is also not evaluable when something in front of the server answered in its place: a 401, a 403 that reads as an auth refusal (an auth gate: without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after one resend after Retry-After (capped at 2 s), or any other 403 that a conformant server/discover sent next to the probe could not get past either. That twin is sent through the suite's client (the same credential and protocol headers) only for such a 403, is resent once on a 429, and credits the 403 only when it was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429); when it drew the same 403, a message naming Host/Origin validation is quoted. A 5xx without -32700 fails as the server failing on the probe rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable. A -32700 on a 5xx passes, with a warning that a 4xx is expected.
- **Pass criteria:** The body '{not json' draws a JSON-RPC error (-32700 expected) or an HTTP 4xx status. A 429 is resent once after Retry-After (capped at 2 s), a -32700 on a 5xx passes with a warning that a 4xx is expected, and any 403 that is not an auth refusal (one without a Bearer challenge, or with --auth one whose challenge carries no error parameter) is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** A hang, a result, or a status below 400 without a JSON-RPC error (an HTML page on a 200). A server that rejects server/discover too fails as not evaluable, and so does an answer something in front of the server gave: a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after its one resend, or any other 403 the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer). A 429 is not resent while server/discover was rejected. A 5xx without -32700 fails as the server failing on the probe rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable.
...
- **Transports:** http
- **Spec reference:** [basic/index#error-codes](https://modelcontextprotocol.io/specification/2026-07-28/basic/index#error-codes)
- **Description:** Checks that the response to the invalid-JSON body carries exactly code -32700 (Parse error), the JSON-RPC 2.0 code MCP reuses for unparsable input. A bare 400 with no JSON-RPC body passes with a warning; a different error code fails. A rejection is credited only when the conformant server/discover was served; a server that rejects everything fails this test as not evaluable (its probe is not resent after a 429 then). A 4xx without a JSON-RPC body is credited only when it is the server's own: a 429 is resent once after Retry-After (capped at 2 s), and a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter), a 429 still a 429 after the resend, or any other 403 that a conformant server/discover sent next to the probe could not get past either (a rate limiter's, an auth gate's or a WAF's bodiless refusal) fails as not evaluable. That twin is sent only for such a 403, so an answer carrying -32700 costs no extra request.
- **Pass criteria:** The invalid-JSON response carries exactly code -32700; a 4xx with no JSON-RPC body passes with a warning when it is the server's own: a 429 is resent once after Retry-After (capped at 2 s), and any 403 that is not an auth refusal (one without a Bearer challenge, or with --auth one whose challenge carries no error parameter) is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** A JSON-RPC error with a code other than -32700, a result, or no JSON-RPC error on a status below 400 or a 5xx. A server that rejects server/discover too fails as not evaluable (its probe is then not resent after a 429), and so does a 4xx without a JSON-RPC body that something in front of the server gave (a rate limiter's, an auth gate's or a WAF's bodiless refusal): a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after its one resend, or any other 403 the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer).
...
- **Transports:** http
- **Spec reference:** [basic/index#error-codes](https://modelcontextprotocol.io/specification/2026-07-28/basic/index#error-codes)
- **Description:** Checks that the malformed-envelope response carries exactly code -32600 (Invalid Request). A bare 400 without a JSON-RPC body passes with a warning; -32601 or -32602 for a message that has no method at all fails. A rejection is credited only when the conformant server/discover was served; a server that rejects everything fails this test as not evaluable (its probe is not resent after a 429 then). A 4xx without a JSON-RPC body is credited only when it is the server's own: a 429 is resent once after Retry-After (capped at 2 s), and a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter), a 429 still a 429 after the resend, or any other 403 that a conformant server/discover sent next to the probe could not get past either (a rate limiter's, an auth gate's or a WAF's bodiless refusal) fails as not evaluable. That twin is sent only for such a 403, so an answer carrying -32600 costs no extra request.
- **Pass criteria:** The malformed-envelope response carries exactly code -32600; a 4xx with no JSON-RPC body passes with a warning when it is the server's own: a 429 is resent once after Retry-After (capped at 2 s), and any 403 that is not an auth refusal (one without a Bearer challenge, or with --auth one whose challenge carries no error parameter) is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** A JSON-RPC error with a code other than -32600 (for example -32601 or -32602), a result, or no JSON-RPC error on a status below 400 or a 5xx. A server that rejects server/discover too fails as not evaluable (its probe is then not resent after a 429), and so does a 4xx without a JSON-RPC body that something in front of the server gave (a rate limiter's, an auth gate's or a WAF's bodiless refusal): a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after its one resend, or any other 403 the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer).
...
- **Description:** Calls tools/call with params carrying only _meta (no name) and expects a JSON-RPC error, -32602 Invalid params expected. A request that fails the CallToolRequest schema is a protocol error and must not produce a result. Skipped when the server declares no tools. An error is credited only when the server wrote it; it fails as not evaluable when something in front of the server answered in its place: a 401, a 403 that reads as an auth refusal (an auth gate: without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after one resend after Retry-After (capped at 2 s), or any other 403 that a conformant server/discover sent next to the probe could not get past either. That twin is sent through the suite's client (the same credential and protocol headers) only for such a 403, is resent once on a 429, and credits the 403 only when it was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429); when it drew the same 403, a message naming Host/Origin validation is quoted. A 5xx without -32602 fails as the server failing on the probe rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable. A -32602 on a 5xx passes, with a warning that a 4xx is expected.
- **Pass criteria:** tools/call with params carrying only _meta (no name) draws a JSON-RPC error (-32602 expected). A 429 is resent once after Retry-After (capped at 2 s), a -32602 on a 5xx passes with a warning that a 4xx is expected, and any 403 that is not an auth refusal (one without a Bearer challenge, or with --auth one whose challenge carries no error parameter) is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429). Skipped when the server declares no tools.
- **Fail criteria:** A result (isError: true included), no JSON-RPC error, or no response. An error the server did not write fails as not evaluable: a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after its one resend, or any other 403 the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer). A 5xx without -32602 fails as the server failing on the probe rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable.
...
- **Default required:** No
- **Spec reference:** [server/discover#discoverresult](https://modelcontextprotocol.io/specification/2026-07-28/server/discover#discoverresult)
- **Description:** Calls the list method (tools/list, resources/list, prompts/list) for every capability the discover result did NOT declare and expects a JSON-RPC error, -32601 expected. Capabilities are the contract for which methods exist; serving an undeclared one means clients cannot trust the discover result. Skipped when every capability is declared. Without a served server/discover no capability counts as declared and no answer can be judged against a declaration, so the list methods are still probed (a 429 not resent) and their answers recorded, but the test fails as not evaluable. With one, a rejection counts only when it is the server's own: each method's 429 is resent once after Retry-After (capped at 2 s), and a 401, a 403 that reads as an auth refusal (an auth gate: without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after the resend, or any other 403 that a conformant server/discover could not get past either fails as not evaluable, the methods grouped by reason, those with the same answer named together. That twin is sent at most once for all the methods, through the suite's client (the same credential and protocol headers) and only for such a 403, is resent once on a 429, and credits the 403 only when it was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429). A 5xx without -32601 fails as the server failing on the method rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable; when the details cannot hold every group, that failure is named first. A -32601 on a 5xx passes, with a warning for each such method that a 4xx is expected.
- **Pass criteria:** Every list method for a capability the discover result did not declare draws a JSON-RPC error (-32601 expected) or a rejecting status, as the server's own rejection. Each method's 429 is resent once after Retry-After (capped at 2 s), a -32601 on a 5xx passes with a warning that a 4xx is expected, and any 403 that is not an auth refusal (one without a Bearer challenge, or with --auth one whose challenge carries no error parameter) is read as the server's only when a conformant server/discover, sent at most once for all the methods (a 429 on it resent once), was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429). Skipped when every capability is declared.
- **Fail criteria:** A list method for an undeclared capability returns a result or gets no response. Without a served server/discover the test fails as not evaluable (the answers are still recorded, a 429 not resent); with one, so does an answer something in front of the server gave, the methods grouped by reason and those with the same answer named together: a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after its one resend, or any other 403 the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer). A 5xx without -32601 fails as the server failing on the method rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable. When the details cannot hold every group, a method the server failed on is named first, and is never called not evaluable.
...
- **Description:** Sends a garbage cursor to the first list method the server supports and expects either a JSON-RPC error (-32602 expected, which is what the pagination page says an invalid cursor SHOULD produce) or a valid first page. A rejection is credited only when it is the server's own: a 429 is resent once after Retry-After (capped at 2 s), and a 401, a 403 that reads as an auth refusal (an auth gate: without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after the resend, or any other 403 that the same list method, sent without the cursor next to the probe, could not get past either (so a gateway or ACL that lets server/discover through but refuses the list method itself is not credited) fails as not evaluable. That twin is sent through the suite's client (the same credential and protocol headers) only for such a 403, is resent once on a 429, and credits the 403 only when it was served or drew a status the server itself chose (a 2xx, or a 4xx other than 401, 403 or 429); when it drew the same 403, a message naming Host/Origin validation is quoted. A 5xx without -32602 fails as the server failing on the cursor rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable. A -32602 on a 5xx is credited, with a warning that a 4xx is expected. A result on a 5xx, a crash, or a malformed result fails.
- **Pass criteria:** A garbage cursor on the first supported list method draws a JSON-RPC error (-32602 expected) or a valid first page. A 429 is resent once after Retry-After (capped at 2 s), a -32602 on a 5xx passes with a warning that a 4xx is expected, and any 403 that is not an auth refusal (one without a Bearer challenge, or with --auth one whose challenge carries no error parameter) is read as the server's only when the same list method sent without the cursor next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** A result on a 5xx status, a crash (no response), a result without the list's array, or neither an error nor a result. An error or rejecting status fails as not evaluable when it is a 401, a 403 that reads as an auth refusal (without --auth any Bearer challenge, with --auth one carrying an error parameter; a gateway's -32001 body included), a 429 still a 429 after its one resend, or any other 403 the same list method sent without the cursor could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer), so a gateway or ACL that lets server/discover through but refuses the list method itself is not credited. A 5xx without -32602 fails as the server failing on the cursor rather than rejecting it (a broken server, or a gateway with no backend), not as not evaluable.
