...
- **Pass criteria:** HTTP 4xx (415 or 400 expected) for an otherwise valid server/discover sent with Content-Type: text/plain, as the server's own rejection. A 429 is resent once after Retry-After (capped at 2 s), and a 403 without a Bearer challenge is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** The text/plain body is parsed and answered with a 2xx status, or a redirect. A rejecting status fails as not evaluable when it is a 401, a 403 carrying a Bearer challenge (a gateway's -32001 body included), a 429 still a 429 after its one resend, any 5xx, or a 403 without a Bearer challenge the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer).
...
- **Pass criteria:** A JSON array of two valid server/discover requests draws HTTP 4xx or a JSON-RPC error body, as the server's own rejection. A 429 is resent once after Retry-After (capped at 2 s), a -32600 on a 5xx passes with a warning that a 4xx is expected, and a 403 without a Bearer challenge is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** A 2xx status without a JSON-RPC error, an array response (a JSON array body, or an SSE data frame holding an array), or a text/event-stream carrying no JSON-RPC response (notifications before an error frame are ignored). A rejecting status fails as not evaluable when it is a 401, a 403 carrying a Bearer challenge (a gateway's -32001 body included), a 429 still a 429 after its one resend, a 5xx without -32600, or a 403 without a Bearer challenge the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer).
...
- **Pass criteria:** The server/discover response has jsonrpc exactly '2.0', the request id echoed, and exactly one of result (an object) or error. Over HTTP a discover answered without a result that drew a 429 is resent once after Retry-After (capped at 2 s) and the second answer is judged; a server's own refusal code (-32600, -32601, -32602, -32020, -32021 or -32022) on a 5xx passes with a warning that a 4xx is expected.
- **Fail criteria:** A missing or wrong jsonrpc field, a missing id, both or neither of result/error, or a non-object result. Over HTTP a discover answered without a result fails as not evaluable when something in front of the server wrote the envelope: a 401, any 403 (it refused the conformant request itself; a message naming Host/Origin validation is quoted), a 429 still a 429 after its one resend, or a 5xx without one of those codes. A resend that gets no response is server unreachable.
...
- **Pass criteria:** tools/call of the first tool without required arguments (tools/list fetched on demand) with _meta.progressToken completes, and every notifications/progress observed for it carries the same token with a strictly increasing progress value (no notifications at all also passes). A 429 on any of the check's calls is resent once after Retry-After (capped at 2 s). A call that fails on the server's side passes as an observation when the failure is not the token's: the server's own answer to the same call without the token, sent right after, was not served either, or the call carrying the token, resent after that, was answered without failing again (a cold backend's first call passes on the served resend, whose notifications are judged the same way). Skipped when the server declares no tools; skipped pointing at tools-list when tools/list failed and that rule is in the run; and skipped as measuring nothing about the token when a gate answered one of the calls (a 429 still a 429, a 401, a Bearer-challenge 403), the call without the token got no response, or the first or resent call got none (a timeout, a connection never established, a stdio child already gone).
- **Fail criteria:** A progress notification carrying a foreign token, or a non-numeric or non-increasing progress value (on the call or on its resend); a call carrying _meta.progressToken that fails on the server's side (a JSON-RPC error, an HTTP status >= 400 other than a 429 or an auth gate's 401 / Bearer-challenge 403, a connection the server closed, or a stdio exit, after which the child is restarted with a warning) while the same call without it, sent right after, is served and the call carrying it fails again when resent; or tools/list failed while tools-list was filtered out of the run (the recorded reason is named).
...
Error tests send conformant envelopes so the error under test comes from the server's own dispatch, not from `_meta` validation. `error-unknown-method` now expects HTTP 404 on the JSON-RPC error. Two rules are post-hoc scans of every JSON-RPC error recorded during the run; a body that is not a jsonrpc 2.0 error (an auth gate's `{"error":"invalid_token"}` on 401, a gateway's `{"error":{"code":400,...}}`) is not counted. The rules that credit any rejection (`error-unknown-method`, `error-invalid-jsonrpc`, `error-invalid-json`, `error-missing-params`, `error-capability-gated`, `error-invalid-cursor`) also ask whose answer it is, so a gateway's 401 with a `-32001` body that echoes the id does not pass them: a 401, a 403 carrying a Bearer challenge, a 429 still a 429 after one resend, a 5xx without the rule's own code (`-32601`, `-32600`, `-32700`, `-32602`, `-32601`, `-32602`), or a 403 without a Bearer challenge that a conformant `server/discover` sent next to the probe could not get past either fails as not evaluable, and the rule's own code on a 5xx is credited with a warning about the status. The exact-code rules credit only the one right code, which no gate produces. `tools-call-unknown` is not read this way yet: a gateway's `-32001` on its `tools/call` still passes it.
...
- **Pass criteria:** A JSON object with no method and no id draws a JSON-RPC error or an HTTP 4xx status. A 429 is resent once after Retry-After (capped at 2 s), a -32600 on a 5xx passes with a warning that a 4xx is expected, and a 403 without a Bearer challenge is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** A result, a status below 400 without a JSON-RPC error, or no response. A server that rejects server/discover too fails as not evaluable, and so does an answer something in front of the server gave: a 401, a 403 carrying a Bearer challenge (a gateway's -32001 body included), a 429 still a 429 after its one resend, a 5xx without -32600, or a 403 without a Bearer challenge the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer).
...
- **Pass criteria:** The body '{not json' draws a JSON-RPC error (-32700 expected) or an HTTP 4xx status. A 429 is resent once after Retry-After (capped at 2 s), a -32700 on a 5xx passes with a warning that a 4xx is expected, and a 403 without a Bearer challenge is read as the server's only when a conformant server/discover sent next to it (a 429 on it resent once) was served or drew a status the server chose (a 2xx, or a 4xx other than 401, 403 or 429).
- **Fail criteria:** A hang, a result, or a status below 400 without a JSON-RPC error (an HTML page on a 200). A server that rejects server/discover too fails as not evaluable, and so does an answer something in front of the server gave: a 401, a 403 carrying a Bearer challenge (a gateway's -32001 body included), a 429 still a 429 after its one resend, a 5xx without -32700, or a 403 without a Bearer challenge the conformant server/discover could not get past either (it drew the same 403, a 401, a 429 again, a 5xx, a redirect or no answer).
