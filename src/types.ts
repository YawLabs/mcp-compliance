...
export const TEST_DEFINITIONS: TestDefinition[] = [
  ...
  },
  {
    ...
    specRef: "basic/transports#streamable-http",
    description:
      "Sends a POST with Content-Type: text/plain instead of application/json and verifies the server rejects it with a 4xx status. Only the server's own rejection counts: a 401 or an auth-gate 403 (one whose Bearer challenge reads as an auth refusal) and a 429 are answers something in front of the server gave in its place, and each fails as not evaluable. A 429 is first resent once after Retry-After, capped at 2 s, and the second answer decides. Any other 403 counts only when the same ping sent on its own as application/json was served or drew a different status, whatever its message says; that ping is sent only in this case (at most once per run, shared with transport-batch-reject and lifecycle-jsonrpc). When that ping drew the same 403, or got no answer, the probe fails as not evaluable, quoting the message when the ping drew the same 403 and the message names Host/Origin validation. A 5xx fails as the server failing on the request rather than refusing it.",
    recommendation:
  ...
  },
  {
    ...
    specRef: "basic",
    description:
      "Sends invalid JSON and verifies the server returns a parse error (-32700) or 4xx status code. The answer is read as error-invalid-jsonrpc reads its probe: a 429 is resent once, and a rejection the unserved initialize handshake drew too (same status, or no answer), a 401 or an auth-gate 403, a 429 on the resend as well, or any other 403 when a well-formed ping sent with the same headers drew the same 403 or got no answer fails as not evaluable. A 5xx fails as the server failing on the request rather than rejecting it, unless it carries the server's own -32700 (Parse error): that passes, with a warning that a rejected request should get a 4xx. A caller's abort is rethrown.",
    recommendation:
