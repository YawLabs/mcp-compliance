...
import type { Recorder, SentRequest } from "../recorder.js";
...
/**
 * Where each finished HTTP exchange ends in its recording: the exchange's
 * sent entry -> the last recorder seq issued before the client stopped
 * reading its response. Over HTTP a message can only arrive on the
 * response of an exchange the client is still reading -- request() emits
 * a body's messages before it returns, raw() records its body before it
 * returns, a notification's body is never recorded, and a held-open
 * stream emits only while its iterator runs -- so nothing recorded after
 * that seq answers the exchange. The post-hoc timeline reads this to rule
 * out a closed `subscriptions/listen` (or any other finished exchange) as
 * the owner of a later id-less reply. stdio has no such boundary (a reply
 * can arrive at any time) and never gets an entry; a stdio stream's
 * close() is marked by the notifications/cancelled it records instead.
 *
 * Keyed by the recorder's own entries, so it lives exactly as long as the
 * recording and covers every client that writes to one.
 */
const exchangeEnds = new WeakMap<SentRequest, number>();

/** The last recorder seq that can belong to a finished HTTP exchange (see exchangeEnds); undefined while it is open, and on stdio. */
export function exchangeEndOf(sent: SentRequest): number | undefined {
  return exchangeEnds.get(sent);
}
...
export function createModernClient(options: ModernClientOptions): ModernClient {
  ...
  /** Mark an HTTP exchange finished at the recording's current end (see exchangeEnds); first mark wins. */
  function endExchange(sent: SentRequest | undefined): void {
    if (!sent || transport.kind !== "http" || exchangeEnds.has(sent)) return;
    const lastSent = recorder.sent[recorder.sent.length - 1]?.seq ?? -1;
    const lastReceived = recorder.received[recorder.received.length - 1]?.seq ?? -1;
    exchangeEnds.set(sent, Math.max(lastSent, lastReceived));
  }

  /**
   * An HTTP stream whose end is marked when the caller closes it or its
   * message iterator finishes (the server ended it, the timeout or an
   * abort cut it, or the caller broke out of the loop), whichever is
   * first. stdio streams pass through: their close() records a cancel.
   */
  function trackStream(stream: TransportStream, sent: SentRequest | undefined): TransportStream {
    if (transport.kind !== "http" || !sent) return stream;
  ...
  const client: ModernClient = {
    ...
    async rpc(method, params, opts = {}) {
      ...
      const nextId = () => {
        const issued = opts.id ?? options.nextId();
        sent = recorder.recordSent({ id: issued, method, params: finalParams, meta: metaOfParams(finalParams) });
        return issued;
      };
      try {
    ...
    async notify(method, params, opts = {}) {
      ...
      const headers = transport.kind === "http" ? headersFor(method, finalParams, opts) : undefined;
      const sent = recorder.recordSent({ id: undefined, method, params: finalParams, meta: metaOfParams(finalParams) });
      try {
    ...
    async stream(method, params, opts = {}) {
      ...
      const nextId = () => {
        ...
        sent = recorder.recordSent({ id, method, params: finalParams, meta: metaOfParams(finalParams) });
        return id;
      };
      let stream: TransportStream;
      try {
        stream = await transport.stream(method, finalParams, nextId, {
          timeout: opts.timeout ?? options.timeout,
          headers,
          omitUserHeaders: opts.omitUserHeaders,
          signal: opts.signal ?? options.signal,
      ...
      return trackStream(stream, sent);
    },
    async raw(body, opts = {}) {
      const t = http();
      ...
      const sent = recorder.recordSent({
        id: undefined,
        method: opts.method ?? "",
        params: undefined,
        meta: undefined,
...
/** A JSON-RPC error object as the modern suite reads it (see errorOf). */
export interface JsonRpcErrorInfo {
  /** The code when it is a number, else NaN: compares unequal to every expected code. */
  code: number;
  /** The `code` member exactly as sent (undefined when missing), for details: render it with errorCodeText / errorWithCode. */
  rawCode: unknown;
  message: string;
  data?: unknown;
}
...
export function errorOf(body: unknown): JsonRpcErrorInfo | undefined {
...
export function describeResponse(res: RpcResponse): string {
  ...
  if (err) return `${errorWithCode(err.rawCode)}${err.message ? ` (${err.message})` : ""}`;
