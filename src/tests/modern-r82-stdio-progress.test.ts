...
import { LIVENESS_BUDGET_MS } from "../suites/modern/liveness.js";
 ...
 * short server/discover (checkLiveness, liveness.ts): a child found gone
 * makes that call a dropped one, charged to it ("made the server exit
 * (after answering it, ...)"), and is replaced before the next call; a
 * child that stops answering is named in a warning.
 */
...
const AFTER_ANSWER =
  "made the server exit (after answering it, exit code 3: Error: progress timer fired after the reply)";
 ...
 * with a token (`wedge-after-token`). A server/discover without `_meta`
...
let calls = 0;
let wedged = false;
const exitNow = () => { fs.writeSync(2, "Error: progress timer fired after the reply\\n"); process.exit(3); };
 ...
 */
async function run(mode: string, only: string[] = [PROGRESS]) {
...
describe("2026-07-28 lifecycle-progress-token over stdio: a child that exits right after answering", () => {
  it("a served call carrying the token, then an exit: the exit is charged to that call, reproduced, and the child restarted (before: PASS, left dead)", async () => {
    ...
    expect(verdict).toMatch(/^FAIL: /);
    expect(verdict).toContain(`tools/call count carrying _meta.progressToken ${AFTER_ANSWER}`);
    expect(verdict).toContain(`${AFTER_ANSWER} when it was resent`);
  ...
  it("a process that fails and exits on its first call whatever it carries: not evaluable, never blamed on the token (before: sometimes FAIL)", async () => {
    ...
    expect(verdict).toMatch(/^PASS \(skipped\): /);
    expect(verdict).toContain(`tools/call count ${AFTER_ANSWER}`);
    expect(verdict).toContain(`the same call without the token ${AFTER_ANSWER}`);
    expect(verdict).toContain("not evaluable");
...
describe("2026-07-28 lifecycle-progress-token over stdio: a child that stops answering after the call", () => {
  it("the served call stands, and a warning names it (before: nothing said why the checks after it hang)", async () => {
    const { verdict, calls, progressWarnings, report } = await run("wedge-after-token");
    expect(verdict).toBe(`PASS: tools/call count succeeded; ${NONE}`);
    expect(calls).toEqual([TOKEN]);
    expect(progressWarnings).toEqual([
      `${PROGRESS}: the server stopped answering right after tools/call count carrying _meta.progressToken was answered (no reply to server/discover within ${LIVENESS_BUDGET_MS}ms); it was not restarted, so the tests after it may fail on the same hang.`,
    ]);
    // The follow-up discover waited its short budget, not the 5 s --timeout.
    expect(resultOf(report, PROGRESS).durationMs).toBeLessThan(4500);
  }, 60_000);
});
...
describe("2026-07-28 lifecycle-progress-token over stdio: a healthy server keeps its verdict", () => {
  it("a served call passes after exactly one call, with no warning", async () => {
    ...
    expect(resultOf(report, PROGRESS).durationMs).toBeLessThan(LIVENESS_BUDGET_MS);
