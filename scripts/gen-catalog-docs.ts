...
const C: Record<string, Criteria> = {
  ...
  "security-sql-injection": {
    ...
    fail: `Any result containing database error text or dumped rows that is not also a rejection, or a server that dies on a payload (the stdio child exits, or on HTTP the connection is dropped and a follow-up server/discover is neither served nor refused with 401/403, a 429 counting only when one retry after Retry-After is; a drop the server outlives -- a WAF or IPS, a keep-alive close, or a crash of one worker of a multi-process server while the others still answer, which a client cannot tell apart -- counts as never reached instead). ${LIST_FAILED_FAIL("tools")}`,
    gate: "tools",
  },
  "security-path-traversal": {
    ...
    fail: `Any result containing out-of-scope file contents that is not also a rejection, or a server that dies on a payload (the stdio child exits, or on HTTP the connection is dropped and a follow-up server/discover is neither served nor refused with 401/403, a 429 counting only when one retry after Retry-After is; a drop the server outlives -- a WAF or IPS, a keep-alive close, or a crash of one worker of a multi-process server while the others still answer, which a client cannot tell apart -- counts as never reached instead). ${LIST_FAILED_FAIL("tools")}`,
    gate: "tools",
  },
  "security-ssrf-internal": {
    ...
    fail: `Any result containing metadata or internal-service content that is not also a rejection, or a server that dies on a payload (the stdio child exits, or on HTTP the connection is dropped and a follow-up server/discover is neither served nor refused with 401/403, a 429 counting only when one retry after Retry-After is; a drop the server outlives -- a WAF or IPS, a keep-alive close, or a crash of one worker of a multi-process server while the others still answer, which a client cannot tell apart -- counts as never reached instead). ${LIST_FAILED_FAIL("tools")}`,
    gate: "tools",
  },
